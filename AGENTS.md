...
- **Benchmark results** (19 scenario-strategy combinations × 20 seeds × 1,000 steps = 380 runs, 6.3s wall-clock; GridWorld has no `static_masking` arm — see REPRODUCIBILITY.md):
  ...
  - **Note**: All old benchmark runs were invalidated by the baseline-decoupling bug. Re-ran with fix — benchmarks now show meaningful strategy differentiation. Invalidated a second time by #301: the loop seed was stored in each report's metadata and handed to no scenario, so every std and bootstrap interval published before that fix described twenty repeats of one run — except on the `random` arms of TemptationBank and DriftLab, whose baseline drew from the loop seed then as it does now, and whose figures the fix left unchanged. The figures above come from the re-run after it. Full outputs: `results/benchmark_results.json`, `results/benchmark_summary.csv`, `results/figures/`
